...
bitonic_connect(Xs, NumProcs) ->
    ...
    if
        ...
        true ->
            ...
            Parent = self(),
            R = make_ref(),
            spawn_link(fun() -> Result = bitonic_connect(Maxs, NumProcs div 2), Parent ! {R, Result} end),
            Minsconnected = bitonic_connect(Mins, NumProcs div 2),
...
bitonic_sort(Xs, NumProcs) ->
    ...
    if
        ...
        true ->
            ...
            Parent = self(),
            R = make_ref(),
            spawn_link(fun() -> Result = lists:reverse(bitonic_sort(Right, NumProcs div 2)), Parent ! {R, Result} end),
            LeftSorted = bitonic_sort(Left, NumProcs div 2),
            RightSorted = receive
                {R, Result} -> Result
            end,
